...
const sendEmail = require("../utils/sendEmail"); // Uncomment if using email functionality
...
// @desc    Generate a reset token and (optionally) send an email
exports.forgotPassword = async (req, res) => {
    try {
      ...
      // Make sure CLIENT_URL is defined in your .env file
      const resetLink = `${process.env.CLIENT_URL}/reset-password.html?token=${resetToken}`;
      console.log("CLIENT_URL:", process.env.CLIENT_URL);
      console.log("RESET_URL:", resetLink);
      var tempurl = process.env.CLIENT_URL;

      // HTML email template with a nice design
      const emailHTML = `
        <html>
      ...
      res.json({ message: "Reset link sent to your email" , resetLink ,tempurl });
    } catch (err) {
      console.error(err);
      res.status(500).json({ message: "Server error" });
    }
...
exports.resetPassword = async (req, res) => {
    try {
      ...
      if (!resetToken || !newPassword) {
        return res.status(400).json({ message: "Please provide both a reset token and a new password" });
      }
      ...
      // Find the user using the ID embedded in the token
